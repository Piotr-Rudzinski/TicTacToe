...
// Sprawdza, czy w jednej linii sa trzy znaki 'X'. Jesli tak to konczy i zwraca "X wins".
// Kolumny
        for (int i=0; i<3; i++) {
            lancuch = ""+ grid[i][0] + grid[i][1] + grid[i][2];
            if (lancuch.equals("xxx")) {
                result = "X wins";
                return result;
            }
        }
// Wiersze
        for (int i=0; i<3; i++) {
            lancuch = ""+ grid[0][i] + grid[1][i] + grid[2][i];
            if (lancuch.equals("xxx")) {
                result = "X wins";
                return result;
            }
 ...
 // Przekatne
        lancuch = ""+ grid[0][0] + grid[1][1] + grid[2][2];
        if (lancuch.equals("xxx")) {
            result = "X wins";
            return result;
        }

        lancuch = ""+ grid[2][0] + grid[1][1] + grid[0][2];
        if (lancuch.equals("xxx")) {
            result = "X wins";
            return result;
        }

// Sprawdza, czy w jednej linii sa trzy znaki 'O'. Jesli tak to konczy i zwraca "O wins".
// Kolumny
        for (int i=0; i<3; i++) {
            lancuch = ""+ grid[i][0] + grid[i][1] + grid[i][2];
            if (lancuch.equals("ooo")) {
                result = "O wins";
                return result;
            }
        }
...
// Sprawdza, czy remis. Jesli na zadnym polu nie ma znaku '-' to wszystkie polazostaly zapelnione, ale nie ma zwyciezcy
        for (int licznik1=0; licznik1 < 3; licznik1++) {
            for (int licznik2=0; licznik2<3; licznik2++) {
                if (!(grid[licznik1][licznik2] == '-')) {
                    ilosc = ilosc + 1;
                }
            }
        }
...
// Nie ma zwyciezcy ani remisu, zwraca "None"
        return result;
    }
